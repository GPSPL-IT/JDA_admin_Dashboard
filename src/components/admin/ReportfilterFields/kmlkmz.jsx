...
const KMLKMZFilter = ({
  ...
  return (
    <form className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
      <div>
        <label className="block mb-1 font-medium">{t('reports.labels.kmlFile', 'KML/KMZ File')}</label>
        <input
          type="file"
          accept=".kml,.kmz"
      ...
      <div className="col-span-1 md:col-span-3 flex flex-wrap gap-4 mt-4">
        <button type="button" className="bg-green-600 text-white px-6 py-2 rounded hover:bg-green-700" onClick={onApply}>{t('reports.labels.applyFilter', 'Apply Filter')}</button>
        <button type="button" className="bg-white border border-gray-300 text-gray-700 px-6 py-2 rounded hover:bg-gray-50" onClick={onReset}>{t('reports.labels.resetFilters', 'Reset Filters')}</button>
      </div>
    </form>
  );
};

export default KMLKMZFilter;
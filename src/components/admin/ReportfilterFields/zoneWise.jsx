...
const ZoneWiseFilter = ({
    ...
    return (
        <form className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-6">
            <div>
                <label className="block mb-1 font-medium">{t('reports.labels.zone', 'Zone')}</label>
                <select
                    className="w-full border rounded px-3 py-2"
                    value={filterValues.zone || ''}
                    onChange={e => onChange('zone', e.target.value)}
                >
                    <option value="">{t('reports.labels.selectZone', 'Select Zone')}</option>
                    {zoneOptions.map(opt => (
